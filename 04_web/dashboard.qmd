---
title: " " 
format: 
  dashboard:
    orientation: rows
    scroll: false
    nav-buttons: [github]
    github: https://github.com/Jesus-Javier-code/Practicas_Empresa_CSIC.git
    theme:
      - flatly
logo: images/Banner_Volcana_Grande.png
logo-alt: "CSIC logo."
---

<style>
/* --- RESET COMPLETO --- */
body, html, .dashboard, .dashboard-page {
  margin: 0 !important;
  padding: 0 !important;
  overflow: visible !important;
}

/* --- ELIMINAR BORDES Y ESPACIOS --- */
.panel-card, .cell, .panel-card-body {
  border: none !important;
  box-shadow: none !important;
  padding: 0 !important;
  margin: 0 !important;
  background: transparent !important;
}

/* --- MAPAS AL 100% SIN ESPACIOS --- */
.columns {
  display: flex;
  width: 100% !important;
  margin: 0 !important;
  padding: 0 !important;
  gap: 0 !important; /* Eliminamos el espacio entre columnas */
}

.column {
  flex: 1;
  margin: 0 !important;
  padding: 0 !important;
}

/* --- IFRAMES COMPACTOS Y SIN BORDES --- */
iframe {
  width: 100% !important;
  height: 450px !important;
  border: none !important;
  margin: 0 !important;
  padding: 0 !important;
  display: block !important;
}

/* --- IMÁGENES RESPONSIVAS --- */
img {
  max-width: 80% !important;
  height: auto !important;
  display: block !important;
  margin: 0 auto !important;
}

/* --- ESTILOS DEL HEADER (se mantienen igual) --- */
@import url('https://fonts.cdnfonts.com/css/hippo-lettro');
.navbar-brand::after {
  content: "VOLCANA";
  font-family: 'Hippo Letto', sans-serif;
  color: #413224;
  font-size: 1.8rem;
  letter-spacing: 1px;
  margin-left: 10px;
}
.navbar-brand:not(:after) { display: none; }
.navbar {
  background: rgb(251,235,207) !important;
  border-bottom: 3px solid #413224 !important;
}
.navbar-brand { color: #413224 !important; }
.navbar-nav .nav-link {
  color: #413224 !important;
  margin: 0 10px;
  border-radius: 4px 4px 0 0;
}
.navbar-nav .nav-link.active {
  background-color: #413224 !important;
  color: white !important;
}
.navbar-nav .nav-link:hover:not(.active) {
  background-color: rgba(65,50,36,0.1) !important;
}
</style>

```{python}
#| label: setup-script
#| include: false
import subprocess
subprocess.run(["python", "../02_utils/geometry.py"], check=True)
```


```{python}
#| label: load-packages
from plotnine import *
import great_tables as gt
import pandas as pd
import glob
from IPython.display import Image
import os
import plotly.io as pio
from IPython.display import IFramee
```


# 🏝️ LA PALMA {#la-palma}

## Potencia Radiativa {.panel-tabset}

```{python}
#| label: setup-potencia
#| include: false
import subprocess
subprocess.run(["python", "../02_utils/geometry_2.py"], check=True)
```

```{python}
#| label: setup-potencia
#| include: false
import subprocess
subprocess.run(["python", "../02_utils/geometry_2.py"], check=True)
```

## Mapas {.columns}
### Mapa interactivo Tajogaite {.column}
```{python}
#| label: mapa-tajogaite-main
#| fig-cap: "Mapa interactivo del Volcán de Tajogaite"
IFrame("images/mapa_tajogaite.html", width="100%", height="450px")
```
### Mapa interactivo los alrededores {.column}
```{python}
#| label: mapa-tajogaite-surroundings
#| fig-cap: "Mapa interactivo de los alrededores del Volcán de Tajogaite"
IFrame("images/mapa_region_alrededor.html", width="100%", height="450px")
```

# 🌋 EL TEIDE

```{python}
#| label: teide-analysis
#| fig-cap: "Análisis de Potencia Radiativa - El Teide"
display(Image(filename="images/graficamala.png", width="80%"))
```

# 🕳️ TERREMOTOS

```{python}
#| label: earthquake-trigger-index
#| fig-cap: "Trigger Index - Terremoto"
display(Image(filename="images/thomas_paper.png", width="80%"))
```