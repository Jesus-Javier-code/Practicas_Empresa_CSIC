...
---
<style>
  /* ===== NUEVO ESTILO PARA NAVEGACIÓN CIRCULAR ===== */
:root {
...
.circle-nav-btn {
  width: 80px;  /* Reducido de 60px */
  height: 80px; /* Reducido de 60px */
  border-radius: 50%;
  display: flex;
  align-items: center;
  justify-content: center;
  cursor: pointer;
  box-shadow: 0 4px 8px rgba(0,0,0,0.15); /* Sombra más sutil */
  transition: all 0.2s ease;
  border: 2px solid white;
  background-size: 60%; /* Ajuste para que la imagen no salga de los bordes */
...
#homepage-btn {
  background-color: var(--volcano-color);
  background-image: url('B_images/Logo_Volcana_circular.jpeg');
  background-size: 70%; /* Logo un poco más grande */
}

#la-palma-btn {
  background-color: var(--volcano-color);
  background-image: url('B_images/la_palma_contorno.png');
  background-size: 50%; /* Imagen más pequeña */
  padding: 2px; /* Espacio extra para contorno */
}

#earthquakes-btn {
  background-color: var(--volcano-color);
  background-image: url('B_images/earthquake-icon.png');
  background-size: 60%; /* Icono más pequeño */
}

/* Eliminar fondos de celdas */
.panel-card, .cell-output, .cell, .panel-card-body, .panel-tabset {
  background: transparent !important;
}

/* Ajustar iframes para que ocupen toda la celda */
iframe {
  background: transparent !important;
}

/* Resto de estilos existentes... (mantén todo lo anterior) */
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('la-palma-btn').addEventListener('click', () => {
    window.location.hash = '#la-palma';
  });
  
  document.getElementById('earthquakes-btn').addEventListener('click', () => {
...
#| label: eq-map
#| echo: false
IFrame("B_images/eq_map.html", width="100%", height="600px")
```