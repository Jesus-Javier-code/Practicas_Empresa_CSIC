...
/* CARRUSEL DE TERREMOTOS - Versión compacta */
.earthquakes-carousel {
  position: relative;
  width: 100%;
  height: 120px; /* Mucho más compacto que los 400px de la homepage */
  margin: 0 0 1rem 0;
  overflow: hidden;
  border-radius: 8px;
...
.earthquakes-carousel .carousel-container {
  position: relative;
  width: 100%;
  height: 100%;
}

.earthquakes-carousel .carousel-slide {
  position: absolute;
  width: 100%;
  height: 100%;
  opacity: 0;
...
.earthquakes-carousel .carousel-slide.active {
  opacity: 1;
}

/* Contenido del header más compacto */
.earthquakes-header-content {
  position: absolute;
  top: 50%;
  left: 50%;
  transform: translate(-50%, -50%);
...
/* Eliminar el overlay si no es necesario */
.earthquakes-carousel .carousel-overlay {
  display: none;
}
...
```
<script>
// CARRUSEL OPTIMIZADO (same as homepage)
document.addEventListener('DOMContentLoaded', function () {
  const slides = document.querySelectorAll('#earthquakes .carousel-slide');
  const credits = document.querySelectorAll('#earthquakes .photo-credits');
  let currentIndex = 0;
  let isAnimating = false;
  let intervalId;
  ...
  // Pausa al interactuar
  const container = document.querySelector('#earthquakes .carousel-container');
  if (container) {
    container.addEventListener('mouseenter', () => clearInterval(intervalId));
    container.addEventListener('mouseleave', startCarousel);
  }
