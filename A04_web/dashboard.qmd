...
<form id="my-form">
  <div class="form-group">
    <label for="date_i">Fecha Inicial:</label>
    <input type="date" id="date_i" name="date_i" class="form-control" max="" required>

    <label for="date_f">Fecha Final:</label>
    ...
    <label for="check_all">Radius of search (km):</label>
    <input type="checkbox" id="check_all" name="check_all" required>
...
<script type="module">
  ...
  form.addEventListener("submit", async (event) => {
    ...
    statusElement.textContent = "Cargando script...";
    statusElement.style.display = "block";

    try {
      const response = await fetch("https://raw.githubusercontent.com//Jesus-Javier-code/Practicas_Empresa_CSIC/blob/main");
      const pythonCode = await response.text();

      await pyodide.runPythonAsync(pythonCode);

      await pyodide.runPythonAsync(`get_all_events("${checkAll}")`);
...
async function initializePyodide() {
  ...
  if (!window.pyodide) {
    window.pyodide = await loadPyodide({
      ...
      stderr: text => {
        outputElement.innerHTML += `<div class="text-danger">${text}</div>`;
        outputElement.scrollTop = outputElement.scrollHeight;
      }
    });
    await pyodide.loadPackage(["micropip", "pandas", "numpy"]);
...
document.getElementById('execute-btn').addEventListener('click', async () => {
  ...
  if (radius <= 0) {
    alert("Radius value must be R > 0");
    return;
  }
  ...
  try {
    ...
    const params = {
      date_i: document.getElementById('date_i').value,
      date_f: document.getElementById('date_f').value,
      lat: parseFloat(document.getElementById('latitude').value),
      lon: parseFloat(document.getElementById('longitude').value),
      radius: radius
    };

    statusElement.textContent = "Cargando script Python desde GitHub...";
    statusElement.className = "alert alert-info";

    const response = await fetch("https://raw.githubusercontent.com/Jesus-Javier-code/Practicas_Empresa_CSIC/blob/main/A01_source/B01_4_eq_processing/preprocess.py");
    const pythonCode = await response.text();
    await pyodide.runPythonAsync(pythonCode);
    ...
    statusElement.textContent = "Ejecutando análisis...";

    const result = await pyodide.runPythonAsync(`
      result = get_all_events(date_i, date_f, lat, lon, radius)
      result.to_html() if hasattr(result, "to_html") else str(result)
    `);
});
