...
# Ruta CORRECTA desde A04_web (donde se ejecuta Quarto)
script_path = os.path.normpath(os.path.join(os.getcwd(), "../A01_source/B01_4_eq_processing/process_eq_data.py"))
...
try:
    # Ejecuta desde el directorio del script (importante para paths internos)
    original_dir = os.getcwd()
    script_dir = os.path.dirname(script_path)
    
    if not os.path.exists(script_dir):
        raise FileNotFoundError(f"Directorio del script no existe: {script_dir}")
        
    os.chdir(script_dir)
    
    result = subprocess.run(["python", "process_eq_data.py"],
                          check=True,
                          capture_output=True,
                          text=True)
    ...
    os.chdir(original_dir)  # Vuelve al directorio original
...
try:
    display(HTML(filename="B_images/eq_map.html"))
except Exception as e:
    print(f"Error al cargar el mapa: {str(e)}")
    display(HTML("<p style='color:red'>No se pudo cargar el mapa de terremotos</p>"))
```
...
#| echo: false
try:
    display(HTML(filename="B_images/eq_table.html"))
except:
    display(HTML("""
    <div style="border:2px dashed #ccc; padding:20px; text-align:center;">
        <h3 style="color:red">Tabla no disponible</h3>
        <p>Verifique que el archivo 'B_images/eq_table.html' existe</p>
    </div>
    """))
```
...
#| label: eq-histogram
#| echo: false
IFrame("B_images/eq_histogram.html", width="100%", height="500px")
```